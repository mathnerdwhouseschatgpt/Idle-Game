...
interface UIElements {
  eraDisplay: HTMLElement;
  kIndex: HTMLElement;
  kNext: HTMLElement;
  elapsed: HTMLElement;
  resourcePanel: HTMLElement;
  buildingsPanel: HTMLElement;
  topProducers: HTMLElement;
...
class UI {
  ...
  private definitions: BuildingData[] = [];
  private definitionById = new Map<string, BuildingData>();
  private eras = new Map<number, EraInfo>();
  private eraSections = new Map<number, EraSectionView>();
  private resourceViews = new Map<ResourceKey, ResourceView>();
  private cardViews = new Map<string, CardView>();
  private visibleEraIndices = new Set<number>([1]);
  private formatMode: "standard" | "scientific" = "standard";
  private lastSnapshot: GameSnapshot | null = null;
  private toastTimer: number | null = null;
  private pendingSaveToast = false;
  ...
  private cacheElements(): UIElements {
    return {
  ...
  private async handleWorkerMessage(message: WorkerToMain): Promise<void> {
    switch (message.type) {
      case "ready":
  ...
  private renderEraShells(): void {
    ...
    for (const [eraIndex, info] of Array.from(this.eras.entries()).sort((a, b) => a[0] - b[0])) {
      ...
      details.addEventListener("toggle", () => {
        ...
        if (this.lastSnapshot) this.updateCards(this.lastSnapshot.cards);
      });
  ...
  private ensureEraCards(eraIndex: number): void {
    const section = this.eraSections.get(eraIndex);
    const era = this.eras.get(eraIndex);
    if (!section || !era || section.rendered) return;
  ...
  private clearEraCards(eraIndex: number): void {
    const section = this.eraSections.get(eraIndex);
    const era = this.eras.get(eraIndex);
    if (!section || !era || !section.rendered) return;
  ...
  private createBuildingCard(definition: BuildingData): CardView {
  ...
  private attachEvents(): void {
    ...
    this.elements.formatSelect.addEventListener("change", () => {
      ...
      if (this.lastSnapshot) this.updateFromSnapshot(this.lastSnapshot);
    });
    ...
    this.elements.resetBtn.addEventListener("click", () => {
      ...
      this.visibleEraIndices = new Set<number>([1]);
  ...
  private updateFromSnapshot(snapshot: GameSnapshot): void {
  ...
  private updateHeader(snapshot: GameSnapshot): void {
  ...
  private updateEraSections(snapshot: GameSnapshot): void {
    for (const [eraIndex, section] of this.eraSections.entries()) {
  ...
  private updateCards(cards: CardSnapshot[]): void {
  ...
  private updateTopProducers(snapshot: GameSnapshot): void {
    const fragment = document.createDocumentFragment();
  ...
  private updateMilestones(snapshot: GameSnapshot): void {
    const fragment = document.createDocumentFragment();
    ...
    if (milestone.nextEra) {
      fragment.appendChild(
        createMilestone(
          `Next Era (${milestone.nextEra})`,
          `Reach total production ${formatNumber(milestone.nextEraThreshold, this.formatMode)}/s`,
          milestone.nextEraProgress,
        ),
      );
    } else {
    ...
    fragment.appendChild(
      createMilestone(
        `Next K Threshold (K${milestone.nextK.toFixed(2)})`,
        milestone.energyDelta > 0
  ...
  private handleUnlocks(unlocks: number[]): void {
  ...
  private async handleSavePayload(kind: SaveKind, payload: SerializedGame): Promise<void> {
    if (kind === "export") {
  ...
  private async exportSave(payload: SerializedGame): Promise<void> {
...
function createButton(label: string, id: string, action: string, quantity?: string): HTMLButtonElement {
  ...
  if (quantity) button.dataset.quantity = quantity;
  return button;
}

function createMilestone(title: string, subtitle: string, progress: number): HTMLElement {
...
function formatNumber(value: number, mode: "standard" | "scientific" = "standard"): string {
  ...
  for (const unit of units) {
    if (abs >= unit.value) return `${(value / unit.value).toFixed(2)}${unit.suffix}`;
  }
...
function loadSavedGame(): Partial<SerializedGame> | undefined {
  if (typeof localStorage === "undefined") return undefined;
  for (const key of SAVE_KEYS) {
    const raw = localStorage.getItem(key);
    if (!raw) continue;
    try {
      return JSON.parse(raw) as Partial<SerializedGame>;
    } catch (error) {
...
function saveGame(payload: SerializedGame): void {
  if (typeof localStorage === "undefined") return;
  localStorage.setItem(WRITE_SAVE_KEY, JSON.stringify(payload));
}

function encodeSave(payload: SerializedGame): string {
  return btoa(encodeURIComponent(JSON.stringify(payload)));
}

function decodeSave(code: string): Partial<SerializedGame> {
  return JSON.parse(decodeURIComponent(atob(code.trim()))) as Partial<SerializedGame>;
}
